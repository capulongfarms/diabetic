...
- **Guarded operations:** Most write operations are wrapped in `_guardedAsync()`, which pops the `modal-pin-prompt` dialog and resolves only on correct PIN. Guards are applied once at the end of `init()` via `_applySecurityGuards()`. **Exception — Add vs Edit save:** opening a modal to add a new record requires no PIN, and the Save button in Add mode calls the unguarded `save*()` base function directly. The Save button in Edit mode calls the guarded `save*Edit()` variant (e.g. `saveReadingEdit`, `saveTestEdit`). Each `open*Modal(id)` dynamically sets the button's `onclick` to the appropriate function based on whether `id` is null (add) or non-null (edit).
